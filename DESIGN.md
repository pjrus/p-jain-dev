...
- **Label scale** (mono 600, uppercase, `--tracking-label` 0.08em, `--leading-label` 1.4): **XS** `--text-label` 0.7rem (hero facts, captions, post meta, form labels), **SM** `--text-label-sm` 0.75rem (experience periods, contact links), **MD** `--text-label-md` 0.8rem (stack card titles, back links, experience location). Project cards keep their own tuned sizes: tech-stack heading 0.78rem, tag chips 0.74rem.
